...
import { ArrowBack, Factory, Lock, Person, Business, Language, LocationOn, Description } from "@mui/icons-material";
...
export default function ManufacturerLogin() {
    ...
    const handleLoginSubmit = async (e) => {
        ...
        // TEMPORARY: Password check disabled for frontend development
        // TODO: Re-enable authentication when frontend changes are complete
        if (loginUser.trim()) {
            const role = 'manufacturer';
            setAuth({ user: loginUser, pwd: 'temp', role });
            setLoginUser('');
            setLoginPwd('');
            navigate('/manufacturer', { replace: true });
            return;
        } else {
            setLoginErrMsg('Please enter a username.');
            return;
        }

        /* ORIGINAL CODE - WILL BE RESTORED LATER
        try {
            ...
            if (res?.data.length === 0) {
                ...
                if (role !== 'manufacturer') {
                    setLoginErrMsg('Access denied. Manufacturer credentials required.');
                    return;
                }
    ...
    const handleRegisterSubmit = async (e) => {
        ...
        // TEMPORARY: Registration disabled for frontend development
        // TODO: Re-enable registration when frontend changes are complete
        alert('Registration is temporarily disabled for frontend development. Use login with any username.');
        setTabValue(0);
        return;
    ...
    return (
        <Box
            sx={{
                ...
                position: 'relative',
                overflow: 'hidden',
                background: 'radial-gradient(circle at center, #102a43, #0b1a2e)',
                ...
                <Fade in timeout={800}>
                    ...
                    >
                        ...
                        <Box sx={{ textAlign: 'center', mb: 3, mt: { xs: 2, sm: 0 } }}>
                            ...
                            >
                                <Factory sx={{ fontSize: { xs: '2rem', sm: '2.5rem' }, color: '#fff' }} />
                            </Box>
                            <Typography
                                variant="h2"
                                sx={{
                            ...
                            >
                                Manage Your Products on Blockchain
                            </Typography>
                        </Box>
                            ...
                            <Box component="form" onSubmit={handleLoginSubmit} noValidate sx={{ width: '100%' }}>
                                ...
                                <Button
                                    ...
                                    sx={{
                                        ...
                                        border: 'none',
                                        '&:hover': {
                                            boxShadow: '0 6px 18px rgba(0, 198, 255, 0.6)',
